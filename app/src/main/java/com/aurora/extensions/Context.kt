...
fun Context.accentColor(): Int {
    val color = when (Preferences.getInteger(this, Preferences.PREFERENCE_THEME_ACCENT)) {
        ...
        14 -> R.color.colorAccent14
        15 -> R.color.colorAccent15
        16 -> R.color.colorAccent16
        17 -> R.color.colorAccent17
        18 -> R.color.colorAccent18
        19 -> R.color.colorAccent19
        20 -> R.color.colorAccent20
        else -> if (isSAndAbove()) {
            R.color.colorAccent00
        } else {
            R.color.colorAccent01
        }
    }
...
fun Context.backgroundColor(): Int {
    val color = when (Preferences.getInteger(this, Preferences.PREFERENCE_THEME_TYPE)) {
        ...
        4 -> {
            R.color.colorDarkXBackground
        }

        5 -> {
            R.color.colorDarkordBackground
        }
...
fun Context.isIgnoringBatteryOptimizations(): Boolean {
    return (getSystemService(Context.POWER_SERVICE) as PowerManager).isIgnoringBatteryOptimizations(
        packageName
    )
}
