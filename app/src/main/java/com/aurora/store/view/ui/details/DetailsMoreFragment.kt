        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.dependentApps.collect { list ->
                if (list.isNotEmpty()) {
                    ...
                    val recyclerDependencyItem = view.findViewById<EpoxyRecyclerView>(R.id.recycler_dependency)
                    val recyclerDependencyEmpty = view.findViewById<AppCompatTextView>(R.id.recycler_dependency_empty)
                    recyclerDependencyItem.visibility = View.GONE
                    recyclerDependencyEmpty.visibility = View.VISIBLE
                }
            }
        }
    }
        ...
        binding.recyclerMore.withModels {
            ...
            if (app.infoBadges.isNotEmpty()) {
                add(
                    HeaderViewModel_()
                        .id("badge_header")
                        .title(getString(R.string.details_more))
                )
            ...
            if (app.appInfo.appInfoMap.isNotEmpty()) {
                add(
                    HeaderViewModel_()
                        .id("info_header")
                        .title(getString(R.string.details_info))
                )
