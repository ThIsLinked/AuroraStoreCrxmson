...
class FavouriteFragment : BaseFragment<FragmentFavouriteBinding>() {
    ...
    private val startForDocumentImport =
        registerForActivityResult(ActivityResultContracts.OpenDocument()) {
            if (it != null) {
                importDeviceConfig(it)
            } else {
                toast(R.string.toast_fav_import_failed)
            }
        }
    private val startForDocumentExport =
        registerForActivityResult(ActivityResultContracts.CreateDocument(mimeType)) {
            if (it != null) {
                exportDeviceConfig(it)
            } else {
                toast(R.string.toast_fav_export_failed)
            }
        }
        ...
        binding.toolbar.apply {
            setOnMenuItemClickListener {
                when (it.itemId) {
                    ...
                    R.id.action_export -> {
                        startForDocumentExport.launch("aurora_store_favourites_${Calendar.getInstance().time.time}.json")
                    }
