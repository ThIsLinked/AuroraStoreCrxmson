...
class AppDetailsFragment : BaseFragment<FragmentDetailsBinding>() {
    ...
    internal val detailsClusterViewModel: DetailsClusterViewModel by activityViewModels()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.reviews.collect {
                binding.layoutDetailsReview.epoxyRecycler.withModels {
                    it.take(4).forEach {
                        add(
                            ReviewViewModel_().id(it.timeStamp).review(it)
                        )
                    }
                }
            }
        ...
        // Data Safety Report
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.dataSafetyReport.collect {
                updateDataSafetyViews(it)
            }
        }
    ...
    private fun inflateAppDescription(app: App) {
        binding.layoutDetailDescription.apply {
            if (CommonUtil.addDiPrefix(app.installs) != "NA") {
            ...
            txtSdk.text = getString(R.string.details_targetSdk, app.targetSdk)
    ...
    private fun inflateAppRatingAndReviews(app: App) {
        binding.layoutDetailsReview.apply {
            ...
            layoutUserReview.visibility = if (authProvider.isAnonymous) {
                View.GONE
            } else {
                View.VISIBLE
            }
    ...
    private fun addAvgReviews(number: Int, max: Long, rating: Long): LinearLayoutCompat {
